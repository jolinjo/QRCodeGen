...
版本號由三個數字組成，格式為 `主版本號.次版本號.修補版本號`（例如 `1.0.1`）：

- **MAJOR（主版本號）**：當你做了不相容的 API 修改或重大功能變更時遞增
  - 例如：`1.0.0` → `2.0.0`
  - 適用情況：重大架構調整、破壞性 API 變更、移除功能等

- **MINOR（次版本號）**：當你做了向下相容的功能性新增時遞增
  - 例如：`1.0.0` → `1.1.0`
  - 適用情況：新增功能、新 API 端點、新格式支援等
  - 遞增時，修補版本號重置為 0

- **PATCH（修補版本號）**：當你做了向下相容的問題修正或小改進時遞增
  - 例如：`1.0.0` → `1.0.1`
  - 適用情況：修正 Bug、優化效能、改進 UI/UX、修正文檔錯誤等
  - 最常使用的遞增方式
